 ...
 *
 * Structured source data for the 2025 Winners page. Rendered as plain
 * HTML by page-winners-2025.php and consumed by the ItemList +
 * LocalBusiness JSON-LD builder.
 *
 * TEST SUBSET: trimmed to 20 entries for local testing only. Replace
 * with the verified, developer-ready dataset from Thomas Wolf
 * (117 winners across all 17 categories) before launch. The shape of
 * each entry must stay: array( name, locality ).
 *
 * @package _s
 */

/**
 * Winner dataset grouped by category.
 *
 * Category order defines both the jump navigation order and ItemList
 * positions, so treat this array as the single ordering authority.
 *
